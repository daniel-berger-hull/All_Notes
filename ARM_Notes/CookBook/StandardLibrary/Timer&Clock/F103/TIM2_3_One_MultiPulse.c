 ...
 * @file    Timer3_OnePulse.h
  ...
  * @date    16-May-2017
  * @brief   This code demonstrate the use of the one pulse mode on 2 timers (Timer2 and Timer3).
  *
  * 		 The pulse is sent to a  pin, using the pin's alternate function. For this
  *          example, the Pin 0 & 1 of port B (PB0) is used by Timer 3, while Pin 2 & 3 of
  *          port A is used by Timer 2.  The channel 3 and 4 are used (There are 4 Capture\Compare
  *          channel on those General Timers)
  *
...
void initGPIO()
{
	...
	//enable port A and alternate function
	...
	//init port A pin 2 = TIM2 channel 3 to alternate function push-pull
	//GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	//GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_AF_PP;
