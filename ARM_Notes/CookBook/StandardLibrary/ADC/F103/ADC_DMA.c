/**
  *****************************************************************************
  * @title   adc_dma.c
  * @Description  Show how to use a dma to transfer the value converted by the ADC into
  * 		      a variable into the demo program. For this, the ADC has to be in
  * 		      Continuous conversion (never stops), so that the DMA always has
  * 		      values to transfer.
  *
  ...
  *
  *				  It is relevent to note that in this demo code, there is no explicit
  *				  call to the ADC read (the ADC_GetConversionValue method). The DMA transfer
